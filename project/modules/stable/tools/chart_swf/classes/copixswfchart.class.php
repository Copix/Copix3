...
class CopixSwfChart {
	...
	function __construct ($swf = false, $library = false) {
		if (!$swf) {
			$this->swf_file = _resource ("chart_swf|swfs/charts.swf");
		}else{
			$this->swf_file = $swf;
		}

		if (!$library) {
			$this->library = _resource ("chart_swf|swfs/charts_library");
		}else{
	...
	function getChart($php_source, $width = 400, $height = 250, $bg_color = "666666", $transparent = false) {
      $flash_file = $this->swf_file;
		...
		$html .= "type='application/x-shockwave-flash' pluginspace='http://www.macromedia.com/go/getflashplayer'></embed></object>";
		return $html;
	}

	static function getChartData ($chart = array ()) {
