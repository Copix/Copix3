...
class Wiki {
    ...
    /**
     * Variable contenant la table d'index
     */
    private $_toc = "";

    /**
     * Tableau contenant les notes de bas de pages
     */
    ...
    /**
     * Objet de traitement des plugins
     */
    private $_wikiPlugins;
    
    /**
     * Fonction de rendu de la classe Wiki
     * 
     * @param string content	Contenu à rendre
     * @param boolean canwritespecialtags
     * @param array tags	
     * @return string
     */
    public function render ($content, $canwritespecialtags = true, $tags = array()){
        // On appelle la classe de gestion des plugins : 
	    $this->_wikiPlugins = _ioClass('wikirender|WikiPlugins');
        ...
        foreach ($elements as $elem) {
            if (in_array ($elem->type, $tags) && !$canwritespecialtags){
                ...
                if ($elem->type == "paragraph" && strlen (trim($elem->data)) > 0) {
                    ...
                    $js .= $this->_addCols($elem)."\n";
                    $js .= '</script>';
                }else if ($elem->type == "plugins") {                    
                    $line = $elem->plugin->render($elem->data);
                }
            }
            $content .= $line;
        }
        ...
        $toc = "<div id=\"wiki_toc\"><strong>Index</strong><br />".$this->_toc."</div>";
    ...
    private function _renderHeader($line) {
        ...
        $this->_toc.=$sp."- <a href=\"#$name\">".$matches[2]."</a><br />";
    ...
    public function _prepareElements ($content){
        ...
        for ($j = 0; $j < count($lines); $j++) {
            ...
            } else if ($pluginElement = $this->_wikiPlugins->isPluginMatch ($line)) {
                // Traitements des functions par plugins
                $elements[$i] = $pluginElement;
            }else {
...
/**
 * Classe des cellules de tables du wiki
 *
 */
