...
class ActionGroupDefault extends CopixActionGroup {
	...
	public function processDelete (){
		...
		if (! _request ('confirm', false)){
			return CopixActionGroup::process ('generictools|Messages::getConfirm', 
				array ('message'=>_i18n ('fileexplorer.confirmDeleteFile', implode (', ', $files)),
						'confirm'=>_url ('delete', array ('file'=>$files, 'confirm'=>1)),
						'cancel'=>_url ('default', array ('path'=>CopixFile::extractFilePath ($files[0])))));
		}

		$done = true;
		$error = array ();
		$fileName = '';//en cas de tableau vide dans file, on ne génère pas de notice au retour.
	...
	public function processPermission(){
		...
		$recursive = CopixRequest::exists('recursive');
		$permission = 0;
		...
		foreach ($arOwnerPerms as $offsetOwner => $labelOwner){
			foreach ($arTypePerms as $offsetPerm => $labelPerm){
				$name = $labelOwner.$labelPerm;
				$active = CopixRequest::exists($name) ? 1 : 0;
				if($active){
					$permission = $permission | ($offsetPerm << $offsetOwner);
				}
			}
		}
		if(is_dir($file)&& $recursive){
			self::_recursePermission($file, $permission, $owner, $group);
		}else{
			self::_setPermission($file, $permission, $owner, $group);
		}
		if(is_dir($file)){
			return _arRedirect(_url('fileexplorer||', array('path' => $file)).'#permissions');
		}else{
			return _arRedirect(_url('fileexplorer||show', array('file' => $file)).'#permissions');
		}
	}
	
	private static function _recursePermission($filePath, $permission,$owner, $group){
		self::_setPermission($filePath, $permission, $owner, $group);
	    $dir = dir($filePath);
	    while(($file = $dir->read()) !== false) {
	    	if($file != '..' && $file != '.'){
		        if(is_dir($dir->path.$file)) {
		            self::_recursePermission($dir->path.$file, $permission, $owner, $group);
		        } else {
		        	self::_setPermission($dir->path.$file, $permission, $owner, $group);
		        }
	    	}
	    }
	    $dir->close();
	}
	private static function _setPermission($filePath, $permission,$owner, $group){
