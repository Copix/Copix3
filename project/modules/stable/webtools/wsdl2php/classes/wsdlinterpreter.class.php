...
{
	private $_classesToSkip = array('string', 'array', 'mixed', 'boolean', 'integer', 'double');
	...
	public function __construct($wsdl)
	{
		...
		try {
			...
			$xslFile = _resourcePath('wsdl2php.xsl');
			$xslFile = CopixModule::getPath('wsdl2php').COPIX_RESOURCES_DIR.'wsdl2php.xsl';
			$xslDom->load($xslFile);
	...
	private function _skipClass($name)
	{
		return class_exists($name) || in_array($name, $this->_classesToSkip);
	}
	...
	private function _validateClassName($className, $addToClassMap = true)
	{
		...
		$addToClassMap = $addToClassMap && !$this->_skipClass($validClassName);
	...
	private function _loadClasses()
	{
		...
		foreach ($classes as $class) {
			...
			if($this->_skipClass($class->getAttribute("validatedName"))){
				continue;
			}
	...
	public function savePHP($outputDirectory)
	{
		...
		foreach ($this->_servicePHPSources as $serviceName => $serviceCode) {
			$filename = $outputDirectory.strtolower($serviceName).".class.php";
