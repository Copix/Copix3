...
class ActionGroupAdmin extends CopixActionGroup {
	...
	public function processConfigure () {
		...
		foreach ($ppo->bodyPreviousValues as $key => $value) {
			if ($value->id_tag > count($ppo->body)) {
				$ppo->freetest[] = $value;
			}
		}	
	...
	public function processSave () {
		 ...
		 */
		if (_request('freetest')) {
			foreach (_request('freetest') as $key => $id) {
				if (CopixRequest::get('activation_' . $id) == 'yes' && isset($id)) {
					if (CopixRequest::get('activation_'. $id) &&
						$recordTest->id_test &&
						CopixRequest::get ('name_' . $id) &&
						CopixRequest::get('path_' . $id) ) {
						...
						return _arRedirect(_url ('copixtest_html|admin|save'), array ('error' => 'Erreur les champs * doivent être obligatoirement remplis'));
					}
