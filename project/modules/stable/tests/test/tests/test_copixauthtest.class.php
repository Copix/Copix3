 ...
 * @subpackage	test
 */
class Test_CopixAuthTest extends CopixTest {
	/**
	...
	function testConnection (){
		...
		list (, $first) = $first = each ($responses);
	...
	function testMultipleHandlersSecondFails() {
		_classInclude('test|testuserhandler');
				
		$user = CopixAuth::getCurrentUser();
		CopixConfig::instance()->copixauth_registerUserHandler (array (
			'name'     => 'test|testuserhandler',
			'rank'     => 20, 
			'required' => false,
		));
		...
		$this->assertFalse ($user->isConnectedAs('test|testuserhandler', 1));
		$this->assertFalse ($user->isConnectedAs('test|testuserhandler', $this->_userId));
		$this->assertFalse ($user->isConnectedWith('test|testuserhandler'));
	...
	function testMultipleHandlersBothAllow() {
		...
		$this->assertEquals(array('auth|dbuserhandler', $this->_userId), $user->getIdentity());
		$this->assertEquals(array(array('auth|dbuserhandler', $this->_userId), array('test|testuserhandler', 5)), $user->getIdentities());
				
		$this->assertTrue ($user->isConnectedAs('auth|dbuserhandler', $this->_userId));
		$this->assertFalse ($user->isConnectedAs('auth|dbuserhandler', $this->_userId+1));
		$this->assertTrue ($user->isConnectedWith('auth|dbuserhandler'));

		$this->assertTrue ($user->isConnectedAs('test|testuserhandler', 5));
		$this->assertFalse ($user->isConnectedAs('test|testuserhandler', 6));
		$this->assertTrue ($user->isConnectedWith('test|testuserhandler'));
	...
	function testMultipleHandlersFirstFails() {
		...
		$this->assertEquals(array('test|testuserhandler', 5), $user->getIdentity());
		$this->assertEquals(array(array('test|testuserhandler', 5)), $user->getIdentities());
		...
		$this->assertEquals("test|testuserhandler", $user->getHandler());		
	}
	...
	function testMultipleHandlersBothAllowRankChanged() {
		...
		$this->assertEquals(array('test|testuserhandler', 5), $user->getIdentity());
		$this->assertEquals(array(array('test|testuserhandler', 5), array('auth|dbuserhandler', $this->_userId)), $user->getIdentities());
