 ...
 * @subpackage test
 */
class Test_DAOTest extends CopixTest {
	/**
	 * Contiendra l'identifiant du premier élément crée dans la table testforeignkey
	 * @var int
	 */
	var $_firstIDFK = null;

	/**
	 * Contiendra l'identifiant du premier enregistrement crée dans la tablea testmain.
	 * @var int
	 */
	var $_firstID = null;
	...
	function setUp (){
		...
		$ct->doQuery ('delete from testmain');
		$ct->doQuery ('delete from testforeignkeytype');
		$ct->doQuery ('delete from testautodao');
	}
	...
	function _testCreationEnregistrementsFK (){
		...
		$recordFK = CopixDAOFactory::createRecord ('test|testforeignkeytype');

		//Création de la dao
		$daoFK = CopixDAOFactory::create ('test|testforeignkeytype');
	...
	function _testCreationEnregistrement (){
		...
		$record = CopixDAOFactory::createRecord ('test|testmain');
		//Création de la dao de l'autre table
		$dao = CopixDAOFactory::create ('test|testmain');
	...
	function _testGetEnregistrement (){
		...
		$daoFK = _dao ('test|testforeignkeytype');
	...
	function _testMethodeXml (){
		...
		$dao = _dao ('test|testmain');
		$results = $dao->findByTitleOrderByTitleLimited ('Titre%', 0, 2);
	...
	function testDAOAuto (){
		CopixDAOFactory::getInstanceOf ('testautodao')->findAll ();
	}
	...
	function testDAOConditionGroup() {
		...
		$dao = CopixDAOFactory::create ('testautodao');
		$sp  = CopixDAOFactory::createSearchParams ();

		$record = CopixDAOFactory::createRecord ('testautodao');
		...
		$this->assertEquals (1, $dao->deleteBy ($sp));
			
		$this->assertTrue (count ($dao->findBy ($sp)) == 0);

		//On vide tout et on vérifie le findAll == 0
		$sp = CopixDAOFactory::createSearchParams ();
		$this->assertTrue ($dao->deleteBy ($sp) >= 1);
	...
	function testDAOConditionGroupArray() {
		...
		//on vérifie qu'un findBy avec un tableau vide retourne vide
	...
	function testDAOEmptyGroups (){
		...
		$sp->startGroup ();
		$sp->endGroup ();
		$this->assertEquals (count (_ioDAO ('testautodao')->findBy ($sp)), 4);
	...
	function testSeveralConditionsOnSameField (){
		...
		$sp->addCondition ('titre_test', '=', 'Titre 3', 'or');
		$sp->addCondition ('titre_test', '=', 'Titre 4', 'or');
		$this->assertEquals (count (_ioDAO ('testautodao')->findBy ($sp)), 2);
		...
		$this->assertEquals (count (_ioDAO ('testautodao')->findBy ($sp)), 3);
	}
	...
	function testGroupAndWithFieldOr (){
		...
		$sp->addCondition ('titre_test', '=', 'Titre 4', 'or');
		$sp->endGroup ();
		$this->assertEquals (count (_ioDAO ('testautodao')->findBy ($sp)), 1);
	}
	...
	function testEmptyGroups (){
		...
		->endGroup ();
		$this->assertEquals (count (_ioDAO ('testautodao')->findBy ($sp)),
		1);
	}
	...
	function testDAONoModule (){
		try {
			_ioDAO ('test|testautodao');
			$this->fails ('test|testautodao');
		}catch (Exception $e){	}
	}
	...
	function testDAOAddSQL (){
		...
		->addSQL ('not exists (select * from testautodao where titre_test = :titre_test)', array (':titre_test'=>2038));
		$this->assertEquals (count (_ioDAO ('testautodao')->findBy ($sp)),
		1);
	}
	...
	function testDAORecordIteratorMoreProperties (){
		$this->_testInsertTestData ();
		$elements = _ioDAO ('testautodao')->findAll ();
	...
	function testDAOOverLoadedFieldNames (){
		...
		//Lecture complète
		foreach (_ioDAO ('testmain_overloaded')->findAll () as $values){
	 ...
	 //test de get
	 $record = _ioDAO ('testmain_overloaded')->get ($this->_firstID);
	 $this->assertNotEquals ($record, false);
	 $this->assertNotEquals ($record, null);

	 //test de findBy
	 $this->assertEquals (count (_ioDAO ('testmain_overloaded')->findBy (_daoSP ()->addCondition ('id', '=', $this->_firstID))), 1);
	}

	public function testInsertNotNullWithEmptyValues (){
		//on test que chaine vide passe.
		$record = _record ('testautodao');
		$record->type_test  = '1';
		$record->titre_test = 'Titre X';
		$record->description_test = '';
		$record->date_test = '20060203';
		_dao ('testautodao')->insert ($record);
	...
	public function testInsertNullInIntenger (){
		...
		$results = _dao ('testautodao')->findBy (_daoSp ()->addCondition ('titre_test', '=', 'Titre XXXX'));
	...
	public function testInsertForceAutoincrement (){
		...
		_dao ('testautodao')->insert ($record, true);
	...
	public function testDateTime (){
		_dao ('testdatetime')->deleteBy (_daoSp ());

		$record = _record ('testdatetime');
		$record->date_dtt = '20071121';
		$record->datetime_dtt = '20071129101222';
		$record->time_dtt = '101222';
		_dao ('testdatetime')->insert ($record);
		...
		$recordGot = _dao ('testdatetime')->get ($record->id_dtt);
		$this->assertEquals ($recordGot->date_dtt, '20071120');

		$results = _dao ('testdatetime')->findAll ();
		...
		$results = _dao ('testdatetime')->findBy ($sp);
		$this->assertEquals (count ($results), 1);
		$this->assertEquals ($results[0]->date_dtt, '20071121');
		$this->assertEquals ($results[0]->datetime_dtt, '20071129101222');
		$this->assertEquals ($results[0]->time_dtt, '101222');
	...
	function testDateTimeOverloaded (){
		_dao ('test|testdatetime_overloaded')->deleteBy (_daoSp ());

		$record = _record ('test|testdatetime_overloaded');
		$record->date_dtt = '20071121';
		$record->datetime_dtt = '20071129101222';
		$record->time_dtt = '101222';
		_dao ('test|testdatetime_overloaded')->insert ($record);
		...
		$recordGot = _dao ('test|testdatetime_overloaded')->get ($record->id_dtt);
		$this->assertEquals ($recordGot->date_dtt, '20071120');

		$results = _dao ('test|testdatetime_overloaded')->findAll ();
		...
		//doit récupérer le premier enregistrement
		$sp = _daoSP ()->addCondition ('date_dtt', '=', '20071121');
		$results = _dao ('test|testdatetime_overloaded')->findBy ($sp);
		$this->assertEquals (count ($results), 1);
		$this->assertEquals ($results[0]->date_dtt, '20071121');
		$this->assertEquals ($results[0]->datetime_dtt, '20071129101222');
		$this->assertEquals ($results[0]->time_dtt, '101222');
	...
	function testDateTimeNullable (){
		...
		$readRecord = _dao ('testdatetime')->get ($record->id_dtt);
	...
	public function testDAOAddPropertiesInForeach (){
		$this->_fillData ();
		$results = _dao ('test|testmain')->findall ();
