...
class DBCredentialHandler implements ICopixCredentialHandler {
	...
	public function assert ($pStringType, $pString, $pUser){
		switch ($pStringType){
			...
			case 'userhandler':
				return $this->_userhanler ($pString, $pUser);
				
			default:
				//Si c'est un module inconnu, on vérifie que l'on est admin, si oui c'est ok
	...
	/**
	 * Gestion du type handler
	 *
	 * @param string $pString	la chaine à tester
	 * @param CopixUser $pUser	l'utilisateur dont on teste les droits
	 */
	private function _userhanler ($pString, $pUser) {
		return $pUser->isConnectedWith ($pString);
	}
