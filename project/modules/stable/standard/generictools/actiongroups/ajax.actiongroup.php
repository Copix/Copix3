...
class ActionGroupAjax extends CopixActionGroup {
	...
	public function processGetZone() {
		...
		} elseif(($instanceId = _request('instanceId')) && ($zoneData = $session->get ($instanceId))) {
			list($zone, $params, $requestUrl, $request) = $zoneData;
			//Permet de rajouter (ou écraser) les paramètres qui ont pu etre envoyé directement au load ou au refresh
			$params = array_merge ($params, CopixRequest::asArray ());
			//Rempli le request avec les valeurs qui avait été passé à la page appelante
			CopixRequest::setRequest ($request, CopixRequest::ADD);
			//On change l'url courante, pour mettre l'url ou on a appelé la zone
			CopixUrl::setRequestedUrl ($requestUrl);
			$ppo->MAIN = CopixZone::process ($zone, $params);
		} else {
			_log (_i18n ('messages.ajax.zone.invalidInstanceId', array ($instanceId)), 'debug', CopixLog::INFORMATION);
		}
		return _arDirectPPO ($ppo, 'blank.tpl');
	}
				...
				el.addEvent ('change', function () {
					...
					$$('.check_$id').each ( function (el) {
						if (el.checked) {
							...
							$('hidden_$id').set ('html', $('hidden_$id').innerHTML+'<input type=\"hidden\" name=\"".$name."[]\" value=\"'+el.value+'\" />');
						}
					});
					input.value = value;
				});
