...
class Socket {
    ...
    public function getHttpContent ($url){
       $oRequest = new HTTPRequest ($url);
       return $oRequest->DownloadToString();
    }
    ...
    public function write ($req){
        fwrite($this->_sock, $req);
        return $this;
    }
    
    public function read(){
        $response = "";
        while(is_resource($this->_sock) && $this->_sock && !feof($this->_sock))
            $response .= fread($this->_sock, 1024);
        return $response;
    }
}
