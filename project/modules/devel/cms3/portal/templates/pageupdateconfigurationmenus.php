<form id="configurationMenusForm">
	...
	<table class="CopixVerticalTable">
		...
		<tr <?php _eTag ('trclass', array ('id' => 'mainMenus')) ?>>
			<th>Modules</th>
			<th style="width: 20px"><?php _eTag ('popupinformation', array ('img' => _resource ('img/tools/help.png')), 'Modules pour lesquels seront affichés les menus, sépares par ; .'); ?></th>
			<td>
...
JS;
				...
				<tr <?php _eTag ('trclass', array ('id' => 'menus')) ?>>
					...
					<td colspan="3">
						...
						_eTag ('inputText', array (
							...
							'name' => 'template[' . $type . ']',
							'id' => 'template_' . $type,
							'value' => (isset ($liste_menus[$type])) ? $liste_menus[$type]->template_hem : null
						))
						?>
					</td>
				</tr>
	...
	<?php } ?>
	<div id="configurationmenusFormSaverDiv" style="text-align: center;">
	<?php
	if (HeadingElementCredentials::canModerate ($record->public_id_hei) && !$module) {
		_eTag ('button', array ('id' => 'headingSave', 'img' => 'img/tools/save.png', 'caption' => 'Sauvegarder', 'type' => 'button'));
		CopixHTMLHeader::addJSDOMReadyCode (" $ ('headingSave').addEvent ('click', function () { saveConfigurationMenusForm () });");
	}
	?>
	</div>
</form>
