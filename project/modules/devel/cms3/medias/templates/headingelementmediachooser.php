...
if($ppo->selectedIndex != null){
	...
	$selectedCaption = '<span style="color: red">[média "' . $ppo->askedSelectedIndex . '" introuvable]</span>';
}

ob_start();
$tabs = array(
	$ppo->identifiantFormulaire.'fromcms'=>"Depuis la bibliothèque du CMS",
	$ppo->identifiantFormulaire.'fromcomputer'=>"Créer un média"
);
...
<div id="<?php echo $ppo->identifiantFormulaire; ?>fromcms">
	<h2>Ajouter un média depuis la bibliothèque du CMS</h2>
	<div class="portalGeneralMenu">
		<ul class="portalGeneralMenuList">
			...
			<li>
				<?php echo CopixZone::process('heading|headingbookmark', array('treeId'=>$ppo->identifiantFormulaire, 'filters'=>$ppo->filters, 'public_id'=>$ppo->selectedIndex ? $ppo->selectedIndex : _request('heading', CopixSession::get('heading', _request('editId'))))); ?>
			</li>
		</ul>
	...
	<div class="elementchoosercontentfiles">
		<div style="border-bottom: 1px solid #CCCCCC;height: 180px;overflow-y : auto;" id="elementchoosercontentfiles<?php echo $ppo->identifiantFormulaire; ?>">
		<?php 
		if ($ppo->selectedIndex != null){
			if (empty($ppo->arElementsPreview)){
				echo "Pas de médias dans cette rubrique.";
			} else {
				$ppoApercu = _ppo();
				...
				echo $tpl->fetch('medias|headingelementmediachooserdetails.php');
			}
		} else {
		...
		<div id="contentMediaPreview<?php echo $ppo->identifiantFormulaire; ?>" style="overflow-y : auto;text-align:center; border-top: 1px solid #CCCCCC;margin-top: 3px;padding-top: 3px;">
			<div style="text-align: center;">Aperçu<br />Cliquez sur un média pour afficher son contenu</div>
		</div>
	</div>
	<div class="elementchooserstate">
		...
		<input type='hidden' value='<?php echo CopixUserPreferences::get('heading|mediaChooser', 0); ?>' name="elementchoosercontentfilesview<?php echo $ppo->identifiantFormulaire; ?>" id="elementchoosercontentfilesview<?php echo $ppo->identifiantFormulaire; ?>" />
		<span id="stateelementchooser<?php echo $ppo->identifiantFormulaire; ?>"></span>
		<input type="button" id="filechoosersubmit<?php echo $ppo->identifiantFormulaire; ?>" value="Valider la selection" onclick="addElements('<?php echo $ppo->identifiantFormulaire; ?>', '<?php echo $ppo->id ; ?>', <?php echo $ppo->multipleSelect ? 'true' : 'false'; ?>); $('copixwindowmediachooser<?php echo $ppo->identifiantFormulaire; ?>').fireEvent('close');"/>
		<?php if ($ppo->selectHeading){?>
		<input type="button" id="mediachoosersubmitheading<?php echo $ppo->identifiantFormulaire; ?>" value="Utiliser la rubrique" onclick="addHeading('<?php echo $ppo->identifiantFormulaire; ?>', '<?php echo $ppo->id ; ?>'); $('copixwindowmediachooser<?php echo $ppo->identifiantFormulaire; ?>').fireEvent('close');" <?php echo (isset($selectedElement) && $selectedElement->type_hei != "heading" ? "disabled='disabled'" : ''); ?>/>
		<?php } ?>
	</div>
</div>
<div id="<?php echo $ppo->identifiantFormulaire; ?>fromcomputer">
	<h2>Ajouter un média depuis votre ordinateur</h2>
	<iframe style="border:none;" width="750px" height="500px" src="<?php echo _url("heading|element|prepareCreate", array('type'=>$ppo->filters[0], 'heading'=>_request('heading', CopixSession::get('heading', _request('editId'))), 'then'=>_url('medias|upload|confirmmediaChooser', array('type'=>$ppo->filters[0], 'heading'=>CopixSession::get('heading', _request('editId')))))); ?>"></iframe>
</div>
<?php
...
ob_end_clean ();
?>
<span class="mediaCaptionElementChooser" style="color:black;<?php echo !$ppo->showSelection ? 'display:none;' : ''; ?>" id="libelleElement<?php echo $ppo->identifiantFormulaire; ?>"><?php echo $selectedCaption; ?></span>
<a class="fileChooserMedia" id="clicker<?php echo $ppo->identifiantFormulaire; ?>" href="javascript:void(0);"><img src="<?php echo $ppo->img ? $ppo->img : _resource ('img/tools/open.png'); ?>" /></a>
<?php
_eTag('copixwindow', array('id'=>'copixwindowmediachooser'.$ppo->identifiantFormulaire, 'fixed' => 1, 'clicker'=>'clicker'.$ppo->identifiantFormulaire, 'title'=>'Parcourir', 'modal'=>true), $content);
...
CopixHTMLHeader::addJSDOMReadyCode("
		...
		tree" . $ppo->identifiantFormulaire . " = new MooTreeControl({
			...
			onSelect: function(node, state) {
				...
				if (state) {
					if($('mediachoosersubmitheading".$ppo->identifiantFormulaire."')){
						$('mediachoosersubmitheading".$ppo->identifiantFormulaire."').disabled = (node.data.type_hei == 'heading' ? false : true);
					}
					filepreview(node.data.public_id_hei, '".$ppo->identifiantFormulaire."', '".$ppo->filters[0]."');
		...
		arMediaTreeId.push('" . $ppo->identifiantFormulaire . "'); 
