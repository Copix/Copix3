...
SQL.IO.prototype.init = function(owner) {
	...
	// bouttons de fermeture
	var ids = ['clientsavecancel'];
	for (var i=0;i<ids.length;i++) {
		var id = ids[i];
		var elm = OZ.$(id);
		this.dom[id] = elm;
	...
	OZ.Event.add(this.dom.clientsavecancel, "click", this.bind(this.clientsavecancel));
...
SQL.IO.prototype.clientsavecancel = function () {
...
SQL.IO.prototype.clientsave = function() {
	var xml = this.owner.toXML().replace ('&amp;', '[__AMP__]');
	var url = Copix.getActionURL ('sqldesigner|designer|save');
	var module = document.getElementById ('savemodule').value;
	var file = document.getElementById ('savefile').value;
	if (file == '') {
		alert ('Nom de fichier vide.');
	}
	var data = 'content=' + xml + '&module=' + module + '&file=' + file;
	OZ.Request(url, this.bind(this.clientsavecallback), {method:'post', data:data});
	this.owner.window.close();
}

SQL.IO.prototype.clientsavecallback = function() {
...
SQL.IO.prototype.clientsql = function() {
	var path = Copix.getResourceURL ("sqldesigner|db/"+window.DATATYPES.getAttribute("db")+"/output.xsl");
...
SQL.Designer.prototype.requestLanguage = function() { /* get locale file */
	...
	var url = Copix.getActionURL("sqldesigner|designer|getLocale", urlParams);
...
SQL.Designer.prototype.requestDB = function() { /* get datatypes file */
	...
	var url = Copix.getResourceURL("sqldesigner|db/"+db+"/datatypes.xml");
