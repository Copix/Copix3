...
class Module {
	...
	static public function createModule ($pPath, $pModuleInfos) {
		// vérification sur la disponibilité de ce nom de module
		if (!CopixModule::isAvailable ($pModuleInfos->getName ())) {
			throw new CopixException (_i18n ('createmodule.error.moduleNameNotAvailable', array ($pModuleInfos->getName ())));
		}
		
		// répertoire du module
		$fullPath = $pPath . $pModuleInfos->getName () . '/';
		if (!CopixFile::createDir ($fullPath)) {
			throw new CopixException (_i18n ('createmodule.error.createDir', array ($fullPath)));
		}
		CopixModule::clearCache ();
		
		// module.xml
		self::xmlCreate ($pModuleInfos);
	}
	 ...
	 */
	static public function xmlCreate ($pModuleInfos) {
		$xml = new DOMDocument (self::$_xmlVersion, self::$_xmlEncoding);
		$element = $xml->createElement ('moduledefinition', '');
		$xml->appendChild ($element);
		$xml->save (self::_getModuleXmlPath ($pModuleInfos->getName ()));
		
		self::xmlSetGeneral ($pModuleInfos);
	}
	 ...
	 * @param moduleinfos $pModuleInfos Informations sur le module
	 */
	static public function xmlSetGeneral ($pModuleInfos) {
		$xml = self::_loadModuleXml ($pModuleInfos->getName ());
		...
		$node->setAttribute ('name', $pModuleInfos->getName ());
		
		// version
		if (!is_null ($pModuleInfos->version)) {
			$node->setAttribute ('version', $pModuleInfos->version);
		}
		...
		// node group
		if (!is_null ($pModuleInfos->group->id)) {
			$node = $xml->createElement ('group', '');
			$node->setAttribute ('id', $pModuleInfos->group->id);
			if (!is_null ($pModuleInfos->group->caption)) {
				$node->setAttribute ('caption', $pModuleInfos->group->caption);
			} else if (!is_null ($pModuleInfos->group->captionI18n)) {
				$node->setAttribute ('captioni18n', $pModuleInfos->group->captionI18n);
			} else {
				throw new CopixException (_i18n ('createmodule.error.emptyGroupCaption'));
			}
		}
		$xml->documentElement->getElementsByTagName ('general')->item (0)->appendChild ($node);
	...
	static public function createActionGroup ($pModule, $pActiongroupInfos, $pActionsInfos, $pBeforeAction = null, $pAfterAction = null) {		
		...
		if (is_array ($pBeforeAction) && count ($pBeforeAction) > 0) {
		...
		if (is_array ($pAfterAction) && count ($pAfterAction) > 0) {
		...
		foreach ($pActionsInfos as $actionInfo) {
			...
			// si on veut un credential sur cette action
			if (!is_null ($actionInfo->credential)) {
				$content .= "\t\t" . '_currentUser ()->assertCredential (\'' . $actionInfo->credential . '\');' . "\n";
				$content .= "\t\t" . "\n";
			}
