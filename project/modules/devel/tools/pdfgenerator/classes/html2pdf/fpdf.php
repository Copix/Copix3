			...
			switch ($align) {
				...
				case 'C' :
					foreach ($content as $k => $chunk) {
						...
						if ($arraysize == 1)
							$this->Cell($stringWidth + $margins + $empty, $lineHeight, $chunk, $b, $skipln, $align, $fill, $this->HREF, $this->x); //mono-style line
					...
					switch ($align) {
						...
						case 'C' :
							foreach ($content as $k => $chunk) {
								...
								if ($arraysize == 1)
									$this->Cell($stringWidth + $margins + $empty, $lineHeight, $chunk, $b, 1, $align, $fill, $this->HREF, $this->x); //mono-style line
								elseif ($k == 0) $this->Cell($stringWidth + ($margins / 2) + ($empty / 2), $lineHeight, $chunk, $b, 0, 'R', $fill, $this->HREF); //first part
