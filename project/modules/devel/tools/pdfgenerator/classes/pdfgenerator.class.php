...
if (!defined('COPIX_HTML2PDF_PATH'))
	define('COPIX_HTML2PDF_PATH', CopixModule::getBasePath("pdfgenerator").'pdfgenerator/classes/html2pdf/');
if (!defined('COPIX_FPDF_PATH'))
	define('COPIX_FPDF_PATH', COPIX_HTML2PDF_PATH);
//as require on FPDF librairie
if (!defined('FPDF_FONTPATH'))
	define('FPDF_FONTPATH', COPIX_HTML2PDF_PATH . 'font/');

Copix::RequireOnce (COPIX_HTML2PDF_PATH . '/html2fpdf.php');
 ...
 */
class PDFGenerator extends HTML2FPDF {

	private $img_ressource = array ();
	private $name;
	...
	* @return void
	*/
	public function __construct (CopixPPO $pOptions = null) {
		if(!$pOptions) $pOptions = _ppo();
		$pOptions = $pOptions->merge(_ppo(array(
			'name' => "untitle.pdf",
			'orientation' =>'P',
			'unit' => 'mm',
			'format' => 'a4'
		)));
		$this->setName($pOptions->name);
		parent :: HTML2FPDF($pOptions->orientation, $pOptions->unit, $pOptions->format);
	}
	...
	*/
	public function setHtml($html) {
        //Copix::RequireOnce (COPIX_HTML2PDF_PATH . '/htmlparser.class.php');
		$html = str_replace("\n", "", $html);
