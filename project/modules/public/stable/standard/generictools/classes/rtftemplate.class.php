...
class RTFTemplate {
	...
	*/
	function fetch ($pRTFTemplateFileName){
		...
		$content = file_get_contents ($selector->getPath ($selector->type == 'module' ? COPIX_TEMPLATES_DIR : '').$selector->fileName);
		if ($content !== false) {
			return $this->_fetch ($content);
		}else{
			return false;
		}
	}

	public function fetchPath ($pPath) {
		return $this->_fetch (file_get_contents ($pPath));
	}

	public function _fetch ($pContent) {
		preg_match_all('{\$(\w+)}', $pContent, $out);
		foreach ($out[1] as $nomVariable){
		   $pContent = str_replace ('\{$'.$nomVariable.'\}', (isset ($this->_data[$nomVariable]) ? $this->_data[$nomVariable] : ''), $pContent);
		}
		return $pContent;
	}
