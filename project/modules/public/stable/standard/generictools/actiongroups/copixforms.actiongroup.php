 ...
 * @subpackage	generictools
 */
class ActionGroupCopixForms extends CopixFormActionGroup {
	...
	public function processNewForm () {
		$pRecord = $this->_form->save ();
		$params = array ();
		if ($pRecord != null) {
		    foreach ($this->_form->getDatasource()->getPk() as $pk) {
		        $params[$pk]=isset ($pRecord->$pk) ? $pRecord->$pk : null;
		    }
		    
		}
		if ($this->_form->getOnSuccessUrl () != null) {
			return _arRedirect (_url ($this->_form->getOnSuccessUrl(), $params));
		}
		return _arRedirect (_url ($this->_form->getFormUrl (), $params));
	}
}
?>