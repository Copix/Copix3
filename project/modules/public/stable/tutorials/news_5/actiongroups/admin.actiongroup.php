...
class ActionGroupAdmin extends CopixActionGroup {
	...
	public function beforeAction (){
		_currentUser ()->assertCredential ('module:Ecrire@news_5');
	}
	...
	public function processDelete (){
		...
		if (! ($record = _ioDAO ('news_5')->get (CopixRequest::getInt ('id_news')))){
		...
		if (! _request ('confirm', false, true)){
			...
			_ioDAO ('news_5')->delete ($record->id_news);
		}
	...
	public function processEdit (){
		...
		if ($news_id = CopixRequest::getInt ('id_news')){
			if ($toEdit = _ioDAO ('news_5')->get ($news_id)){
		...
		if ((($toEdit = CopixSession::get ('news|edit')) === null) || (_request ('new') == 1)){
			CopixSession::set ('news|edit', $toEdit = _record ('news_5'));
		}
		...
		$ppo->arErrors = _request ('errors') ? _ioDAO ('news_5')->check ($toEdit) : array ();
	...
	public function processValid (){
		...
		if (_ioDAO ('news_5')->check ($toEdit) === true){
			if ($toEdit->id_news){
				_ioDAO ('news_5')->update ($toEdit);
			}else{
				_ioDAO ('news_5')->insert ($toEdit);
			}
			//on vide la session
