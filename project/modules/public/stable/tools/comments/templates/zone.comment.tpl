{if $mode eq "list"}
	{if count ($arrComments) neq 0}
		...
		{foreach from=$arrComments item=comment name=comments}
			<dt><a name="#c{$comment->comment_id}" href="{copixurl dest="#"}#c{$comment->comment_id}"" class="comment-id">{$smarty.foreach.comments.iteration}</a> {$comment->date_comment|datetimei18n:text}, {if $comment->authorsite_comment}<a href="{$comment->authorsite_comment}">{/if}{$comment->authorlogin_comment|escape}{if $comment->authorsite_comment}</a>{/if}
			{if $isAdmin}
