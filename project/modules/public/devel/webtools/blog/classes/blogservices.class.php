...
class BlogServices{
	...
	public function getPanel(){
		...
		$manager->add("blog|blogthread",_ppo(array('get'=>'calendar',"more"=>$ar)));
		$manager->add("blog|blogthread",_ppo(array('get'=>'headings')));
		$manager->add("blog|blogthread",_ppo(array('get'=>'tags')));
		$responses = $manager->execute();

		$panel = _ppo();
		foreach ($responses as $response){
			if(_request('debug')){
				echo "réponse: "; print_r($response);
			}
			$panel = $panel->merge($response);
		}
		/**/
		return $panel;
	}
	...
	public function getCalendar($y,$m,$d){
		...
		$p = _ppo(array('generated','calendar'=>array()));
	...
	public function getTags(){
		...
		return (string)CopixCache::read('tags','blogtags');
	}
