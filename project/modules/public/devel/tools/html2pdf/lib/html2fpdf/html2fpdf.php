...
//Modified by Patrice Ferlet [www.copix.org]

define('HTML2FPDF_VERSION', '3.0(beta)');
//-- Modification by Patrice Ferlet
			...
			if ($i % 2 == 0) {
				...
				elseif ($this->specialcontent) {
					if ($this->specialcontent == "type=select" and $this->selectoption['ACTIVE'] == true) //SELECT tag (form element)
						...
						$this->textbuffer[] = array ("»¤¬" /*identifier*/
						.$this->specialcontent."»¤¬".$e);
				}
				elseif ($this->tablestart) {
					if ($this->tdbegin) {
						$this->cell[$this->row][$this->col]['textbuffer'][] = array ($e, $this->HREF, $this->currentstyle, $this->colorarray, $this->currentfont, $this->SUP, $this->SUB, '' /*internal link*/
						, $this->strike, $this->outlineparam, $this->bgcolorarray);
				...
				elseif ($this->pbegin or $this->HREF or $this->divbegin or $this->SUP or $this->SUB or $this->strike or $this->buffer_on) $this->textbuffer[] = array ($e, $this->HREF, $this->currentstyle, $this->colorarray, $this->currentfont, $this->SUP, $this->SUB, '' /*internal link*/
				, $this->strike, $this->outlineparam, $this->bgcolorarray); //Accumulate text on buffer
				else {
		...
		case 'IMG' :
			...
			if (isset ($attr['SRC'])) {
				...
				elseif ($this->pbegin or $this->divbegin) {
					...
					$this->textbuffer[] = array ("»¤¬" /*identifier*/
					."type=image,ypos=$ypos,{$xpos}width=".$sizesarray['WIDTH'].",height=".$sizesarray['HEIGHT']."»¤¬".$sizesarray['OUTPUT']);
					while ($numberoflines) {
						$this->textbuffer[] = array ("\n", $this->HREF, $this->currentstyle, $this->colorarray, $this->currentfont, $this->SUP, $this->SUB, '' /*internal link*/
						, $this->strike, $this->outlineparam, $this->bgcolorarray);
						$numberoflines --;
					}
				} else {
				...
				if ($sizesarray['X'] < $this->x)
					$this->x = $this->lMargin;
				if ($this->tablestart) {
					$this->cell[$this->row][$this->col]['textbuffer'][] = array ("»¤¬" /*identifier*/
					."type=image,width=".$sizesarray['WIDTH'].",height=".$sizesarray['HEIGHT']."»¤¬".$sizesarray['OUTPUT']);
			...
			if (!$this->tablestart) {
				...
				switch (strtoupper($attr['TYPE'])) {
					case 'CHECKBOX' : //Draw Checkbox
						...
						$width = 4;
						$this->cell[$this->row][$this->col]['textbuffer'][] = array ("»¤¬" /*identifier*/
						."type=input,subtype=$type,width=$width,height=$height"."»¤¬".$text);
...
function printbuffer($arrayaux, $outofblock = false, $is_table = false) {
	...
	for ($i = 0; $i < $array_size; $i ++) {
		...
		if (isset ($vetor[0]) and $vetor[0] { 0 }
			== '»' and $vetor[0] { 1 }
			== '¤' and $vetor[0] { 2 }
