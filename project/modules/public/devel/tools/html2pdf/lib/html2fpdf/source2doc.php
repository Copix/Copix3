...
function source2doc($filename) {
	...
	if ($extends != '')
		$html .= '<b>EXTENDS:</b> '.$extends.'<br />'.endl;
	...
	for ($i = 0; $i < count($vname); $i ++) {
		...
		if (empty ($vtype[$i]))
			$html .= '<font size=2><i>(???)</i></font>'.endl;
		else
			$html .= '<font size=2><i>('.$vtype[$i].')</i></font>'.endl;
		$html .= '</td>'.endl;
	...
	for ($i = 0; $i < count($funcname); $i ++) {
		...
		if ($ret != '' or $desc != '') {
			...
			if ($ret == '')
				$html .= '<font size=2><b>Return:</b> <i>?void?</i></font>'.endl;
			else
				$html .= '<font size=2><b>Return:</b> <i>'.trim($ret).'</i></font>'.endl;
			$html .= '</td>'.endl;
			//@desc column
			$html .= '<td width="70%">'.endl;
			if ($desc == '')
				$html .= '<font size=2><b>OBS:</b> </font>'.endl;
			else
				$html .= '<font size=2><b>OBS:</b> '.trim($desc).'</font>'.endl;
			$html .= '</td>'.endl;
