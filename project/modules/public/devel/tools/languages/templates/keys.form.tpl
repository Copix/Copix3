...
<form action="{copixurl dest="keys|addSection"}" id="addSection" method="post">
...
<form action="{copixurl dest="keys|edit"}" method="post">
<input type="hidden" name="moduleName" id="moduleName" value="{$ppo->moduleName}" />
<input type="hidden" name="file" id="file" value="{$ppo->file}" />
...
<table class="CopixVerticalTable">
  ...
  <tr>
    ...
    <td {$alternate} align="right" width="20px">
      <a href="{copixurl dest="keys|delete" moduleName=$ppo->moduleName filemtime=$ppo->filemtime file=$ppo->file message=$section|cat:'.'|cat:$key}"
...
<br />
<div align="center">
  <input type="submit" value="{i18n key="global.other.edit"}" {if !$ppo->canEditFile}disabled{/if} />
</div>
</form>
{/foreach}
...
<form action="{copixurl dest="keys|add"}" id="addMessage" method="post">
<input type="hidden" name="moduleName" id="moduleName" value="{$ppo->moduleName}" />
<input type="hidden" name="file" id="file" value="{$ppo->file}" />
<input type="hidden" name="filemtime" id="file" value="{$ppo->filemtime}" />
