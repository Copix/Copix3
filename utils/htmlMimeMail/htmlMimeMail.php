    ...
    */
    public function __construct()
    {
        ...
        */
        ...
        $this->headers['X-Mailer'] = 'htmlMimeMail <http://www.phpguru.org/>';
    }
...
//				$smtp = &smtp::connect($this->smtp_params);
                ...
                foreach ($recipients as $recipient) {
                    $addresses = new Mail_RFC822($recipient, $this->smtp_params['helo'], null, false);
					$addresses = $addresses->parseAddressList();
                ...
                foreach ($this->headers as $name => $value) {
                    if ($name == 'Cc' OR $name == 'Bcc') {						
	                    $addresses = new Mail_RFC822($value, $this->smtp_params['helo'], null, false);
						$addresses =  $addresses->parseAddressList();
                        foreach ($addresses as $address) {
                            $smtp_recipients[] = sprintf('%s@%s', $address->mailbox, $address->host);
                        }
                ...
                if (isset($this->return_path)) {
                    ...
                    $from = new Mail_RFC822($this->headers['From']);
					$from =  $addresses->parseAddressList();
...
class iEncodingFactory{
	public static function getIEncoding ($pType){
		switch ($pType){
			case '7bit':
			...
			case 'quoted-printable':
				return new QPrintEncoding ();
		}
		throw new Exception ("Type d'encoding ".$pType." introuvable");
	}
}
?>