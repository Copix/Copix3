...
class CopixCacheFileStrategy implements ICopixCacheStrategy {
	...
	public function read ($pId, $pType, $pExtra) {
		...
		throw new CopixCacheException ('Cache not found : ' . $pId . ' - ' . $pType, 0, array ('cache_id' => $pId, 'cache_type' => $pType));
	}
	...
	public function write ($pId, $pContent, $pType, $pExtra) {
		CopixFile::write ($this->_makeFileName ($pId, $pType, $pExtra), serialize (new CopixSerializableObject($pContent)));
	}
	...
	public function clear ($pId, $pType, $pExtra) {
		if ($pId !== null) {
		    if (file_exists ($this->_makeFileName ($pId, $pType, $pExtra))) {
