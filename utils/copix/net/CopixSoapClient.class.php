...
class CopixSoapClient {
    ...
    public static function get($pSoapServer) {
        ...
        $response = $client->launch (new CopixHTTPClientRequest ($pSoapServer));
        if ((! $response[0]->getError ()) && ($response[0]->getHttpCode () != 404)){
        	return new SoapClient ($pSoapServer, $params);
        }else{
        	throw new CopixException ('Cannot connect to '.$pSoapServer);
        }
    }
    
    /**
     * Permet de vérifier la validité d'un serveur SOAP
     * @param string $pSoapServer Le serveur soap
     * @return bool
     */
    public static function verify ($soapClient) {
       	$check = !is_soap_fault($soapClient);
        return $check;
    }
}