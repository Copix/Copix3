 ...
 * @package copix
 * @subpackage log
 * @author Patrice Ferlet, Steevan BARBOYON
 * @copyright CopixTeam
 * @link http://copix.org
...
/**
 * Affiche les logs dans FireBug
 * 
 * @package	copix
 * @subpackage log
 */
class CopixLogFireBugStrategy extends CopixLogAbstractStrategy {
	/**
	 * Effectue un log
	 *
	...
	public function log ($pProfile, $pType, $pLevel, $pDate, $pMessage, $pExtras) {
		...
		$log = $log + $pExtras;
		CopixSession::push ('copix|log|firebug|' . $pProfile, $log);
	}
	...
	public function get ($pProfile, $pStart = 0, $pCount = null) {
		if (is_array ($profile = CopixSession::get ('copix|log|firebug|' . $pProfile))) {
	        $arLog = array ();
	...
	public function delete ($pProfile) {
		CopixSession::set ('copix|log|firebug|' . $pProfile, null);
	}
}