...
abstract class CopixEMail {
	...
	public function __construct ($to, $cc, $cci, $subject, $message){
		...
		$this->to  = is_array ($to)  ? $to : str_replace  (';', ',', $to);
		$this->cc  = is_array ($cc)  ? $cc : str_replace  (';', ',', $cc);
		$this->cci = is_array ($cci) ? $cci : str_replace (';', ',', $cci);
	...
	public function addAttachment ($fileData, $fileName = '', $cType='application/octet-stream', $encoding = 'base64') {	
		...
		$this->attachments[] = new attachment ($fileData, $fileName, $cType, iEncodingFactory::getIEncoding ($encoding));
	}
	...
	public function prepareEmail(htmlMimeMail $mail, $fromAdress, $fromName) {
		...
		if (CopixI18N::getCharset () === 'UTF-8' && extension_loaded("mbstring") && !mb_check_encoding($this->message, "UTF-8")){
...
class CopixHTMLEMail extends CopixEMail {
	...
	function prepareEmail(htmlMimeMail $mailer, $fromAdress, $fromName) {
		...
		$mailer->setTextEncoding(new EightBitEncoding ());
		$mailer->setHtmlEncoding(new EightBitEncoding ());
...
class CopixEMailer {
	...
	private function _createMailer() {
		...
		$mail->setHeadCharset(CopixI18N::getCharset ());
