    ...
    */
    public function addProperty ($field){
        ...
        if($field->fkTable !== null){
            if(! isset ($this->_joinTypes[$field->fkTable])){
               throw new CopixException (_i18n ('copix:dao.error.definitionfile.properties.foreign.table.missing', array ($this->_DAOId, $field->name)));
            }
    ...
    public function addTable ($tableinfos){
        ...
        if (!isset ($tableinfos['name']) || trim ($tableinfos['name']) == ''){
           throw new CopixException (_i18n ('copix:dao.error.definitionfile.table.name'));
        }
        ...
        if (isset ($tableinfos['primary']) && $this->_getBool ($tableinfos['primary'])){
            if($this->_primaryTableName !== null){
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.table.primary.duplicate',$this->_primaryTableName));
            }
            ...
            if (!in_array ($join, array('left','right','inner',''))){
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.table.join.invalid',$tableinfos['NAME']));
            }
    ...
    public function addMethod (&$method) {
        if(isset ($this->_methods[$method->name])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.duplicate', $method->name));
        }
    	...
    	if (Copix::RequireOnce ($this->_phpClassFilePath = $pFilePath)){
    		...
    		if ($pRecordName !== null){
    			if (! class_exists ($pRecordName)){
     	    		throw new CopixException ('Demande d utilisation de  '.$pRecordName.' pour le record mais la classe n est pas définie dans le fichier '.$pFilePath);
    			}else{
    		...
    		if ($pDAOName !== null){
    			if (! class_exists ($pRecordName)){
     	    		throw new CopixException ('Demande d utilisation de  '.$pDAOName.' pour le DAO mais la classe n est pas définie dans le fichier '.$pFilePath);
    			}else{
    		...
    		throw new CopixException ('impossible de charger le fichier de définition demandé pour le DAO '.$pFilePath);
    	}
    }
    ...
    */
    function CopixPropertyForDAO ($params, $def){
        ...
        if (!isset ($params['name'])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.missing.attr', array('name', 'property')));
        }
        ...
        if(!$def->getTable ($this->table)){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.property.unknow.table', array($this->name,$this->table)));
        }
        ...
        if (!isset ($params['type'])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.missing.attr', array('type', 'field')));
        }
        ...
        if (!in_array ($params['type'], array ('autoincrement', 'bigautoincrement', 'int','integer', 'varchar', 'string', 'varchartime', 'time', 'varchardate', 'date',  'datetime', 'numeric', 'double', 'float', 'version'))){
           throw new CopixException (_i18n ('copix:dao.error.definitionfile.wrong.attr', array($this->name,$params['type'], $this->fieldName)));
        }
        ...
        if($this->table == $def->getPrimaryTableName ()){ // on ignore les champs fktable et fkfieldName pour les propriétés qui n'appartiennent pas à la table principale
            ...
            if($this->fkTable !== null){
                if($this->fkFieldName == ''){
                   throw new CopixException (_i18n ('copix:dao.error.definitionfile.property.foreign.field.missing', array($this->name,$this->fkFieldName)));
                } 
            }
        }
...
class CopixMethodForDAO {
    ...
    function CopixMethodForDAO ($method, $def){
        ...
        if (!isset ($attributes['name'])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.missing.attr', array('name', 'method')));
        }
        ...
        if($this->type == 'update'){
            if(isset($method->values) && isset($method->values->value)){
                ...
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.values.undefine',array($this->name)));
            }
        }
        ...
        if (isset($method->limit)){
        	if (count ($method->limit) > 1){
               throw new CopixException (_i18n ('copix:dao.error.definitionfile.tag.duplicate', array('limit', $this->name)));        		
        	}
            ...
            if ($this->type == 'select' || $this->type == 'selectfirst'){
                ...
                if ($offset === null){
                    throw new CopixException (_i18n ('copix:dao.error.definitionfile.missing.attr',array('offset','limit')));
                }
                if ($count === null){
                    throw new CopixException (_i18n ('copix:dao.error.definitionfile.missing.attr',array('count','limit')));
                }
                ...
                if (substr ($offset,0,1) == '$'){
                    if (in_array (substr ($offset,1),$this->_parameters)){
                        ...
                        throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.limit.parameter.unknow', array($this->name, $offset)));
                    }
                }else{
                    if (is_numeric ($offset)){
                        $offset = intval ($offset);
                    }else{
                        throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.limit.badvalue', array($this->name, $offset)));
                    }
                }
                ...
                if (substr ($count,0,1) == '$'){
                    if(in_array (substr ($count,1),$this->_parameters)){
                        ...
                        throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.limit.parameter.unknow', array($this->name, $count)));
                    }
                }else{
                    if(is_numeric($count)){
                        $count=intval($count);
                    }else{
                        throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.limit.badvalue', array($this->name, $count)));
                    }
                }
                $this->_limit= compact('offset', 'count');

            }else{
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.limit.forbidden'));
            }
        }
    }
    ...
    function _addCondition ($attributes){
        ...
        if (!isset ($properties[$field_id])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.property.unknown', array ($this->name, $field_id)));
        }

        if($this->type=='update'){
            if($properties[$field_id]->table != $this->_def->getPrimaryTableName ()){
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.property.forbidden', array($this->name, $field_id)));
            }
        }
        ...
        if (substr($value,0,1) == '$'){
            if (in_array (substr ($value,1),$this->_parameters)){
                ...
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.parameter.unknow', array($this->name, $value)));
            }
        }else{
            if(substr($value,0,2) == '\$'){
    ...
    function addParameter($attributes){
        if (!isset ($attributes['name'])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.parameter.unknowname', array($this->name)));
        }
    ...
    function addOrder($attr){
        ...
        if ($prop != ''){
            if(isset($properties[$prop])){
                ...
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.orderitem.bad', array($prop, $this->name)));
            }
        }else{
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.orderitem.bad', array($prop, $this->name)));
        }
    ...
    function addValue($attr){
        ...
        if ($prop == ''){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.values.property.unknow', array($this->name, $prop)));
        }
        if(!isset($properties[$prop])){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.values.property.unknow', array($this->name, $prop)));
        }
        if($properties[$prop]->table != $this->_def->getPrimaryTableName ()){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.values.property.bad', array($this->name,$prop )));
        }
        if($properties[$prop]->isPK){
            throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.values.property.pkforbidden', array($this->name,$prop )));
        }
        ...
        if (substr($value,0,1) == '$'){
            if (in_array (substr ($value,1),$this->_parameters)){
                ...
                throw new CopixException (_i18n ('copix:dao.error.definitionfile.method.values.unknowparameter', array($this->name, $value)));
            }
        }else{
