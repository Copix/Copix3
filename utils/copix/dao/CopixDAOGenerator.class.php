...
class CopixDAOGenerator {
	...
	*/
	...
	/**
	 * Retourne le code PHP pour le DAO et la classe Record
	 * 
	 * @return string
	 */
	function getPHPCode() {
	...
	private function _prepareValuesForNewDB($fieldList, $prefixfield = '') {
		...
		foreach ((array) $fieldList as $fieldName => $field) {
			...
			} elseif ($driverName == 'pgsql' && $field->type == 'datetime') {
				$values[':' . $fieldName] = 'CopixDateTime::getIsoDateTime ($' . $prefixfield . $fieldName.")";
				$formatted[':' . $fieldName] = ":" . $fieldName;				 
		    } elseif (($driverName == 'mysql') && in_array ($field->type, array ('blob'))){
				//Champs lobs / blob & stuff pour MySql
				$values[':'.$fieldName] = 'base64_encode ($' . $prefixfield . $fieldName.")";
				$formatted[':' . $fieldName] = ":" . $fieldName;
			} else {
				$values[':' . $fieldName] = '$' . $prefixfield . $fieldName;
				$formatted[':' . $fieldName] = ':' . $fieldName;
...
return $toReturn;
	...
	*/
	...
	$database = CopixConfig :: instance()->copixdb_getProfile($this->_definition->getConnectionName())->getDatabase ();
        ...
        foreach ($pFields as $field) {
            ...
            if (in_array ($field->type, array ('date', 'datetime', 'time'))) {
                ...
                $sqlCondition[] = ($pForSelect ? $field->table . '.' : '') . $field->fieldName . " ' . ($fieldValue === null ? 'IS' : '=') . ' ' . ($fieldValue === null ? 'NULL' : '$fieldVarAssign') . ' ";
            }else {
                $array[$fieldVar] = '(' . $fieldValue . ' === null ? "___COPIX___DELETE___ME___FROM____DAO___QUERIES___" : ' . $fieldValue . ')';
                $sqlCondition[] = ($pForSelect ? $field->table . '.' : '') . $field->fieldName . " ' . ($fieldValue === null ? 'IS' : '=') . ' ' . ($fieldValue === null ? 'NULL' : '$fieldVar') . ' ";
            }
        }
	...
	*/
	...
	private function _generatePHP4Get() {
		...
		$result .= $sqlCondition;
		$result .= "';\n"; // ends the query
		$result .= '    $results = new CopixDAORecordIterator ($ct->doQuery ($query, ' . $this->_makeArrayParamsForQuery($arSqlCondition) . '), $this->getDAOId  (), $this->_connectionName);' . "\n";
	...
	private function _generatePHP4FindAll ($pIterator) {
		...
		$result .= '    return new CopixDAORecordIterator (CopixDB::getConnection ($this->_connectionName)->'.$queryMethod.' ($this->_selectQuery), $this->getDAOId  (), $this->_connectionName);' . "\n";
		$result .= " }\n";
		return $result;
	}
...
JOIN;
		...
		$result .= '    return new CopixDAORecordIterator ($ct->doQuery ($query, $params, $searchParams->getOffset (), $searchParams->getCount ()), $this->getDAOId  (), $this->_connectionName);' . "\n";
		$result .= " }\n";
		return $result;
	}
