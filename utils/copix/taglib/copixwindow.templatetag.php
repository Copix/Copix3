...
class TemplateTagCopixWindow extends CopixTemplateTag  {
	...
	public function process ($pParams, $pContent=null){
		...
		$jsParams = array ('dragSelector', 'clicker', 'zone', 'url', 'onComplete', 'onDisplay', 'onFocus', 'onUnfocus');
		
		// Récupère les paramètres
		$zone     = $this->getParam('zone');
		$id       = $this->getParam('id', uniqid());
		$class    = $this->getParam('class', 'popupInformation');
		...
		$js->Copix->register_copixwindow ($id, $jsParams);//->display ();
		_tag ('mootools');
