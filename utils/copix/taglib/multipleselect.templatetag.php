    ...
    */
    public function process ($pParams, $pContent=null) {
	   ...
	   if (!isset($img)) {
		...
		// Si on fait la comparaison des éléments de manière strict ou pas.
       if (!isset ($compStrict) || $compStrict !== false){
			$compStrict = true;
	   }else{
			$compStrict = false;
	   }
	   ...
	   if (isset($selected) && is_array($selected)) {
	        ...
	        $value .= isset ($arValues[$selected]) ? $arValues[$selected] : '';
	    }
	    ...
	    //Div contenant la liste
   	    $toReturn .= '<div id="'.$idDiv.'" style="margin:auto;display:none;top:1px;left:1px;position:absolute;z-index:9999;background-color:white;border:1px solid #bbbbbb">'.$toReturnValue.'</div>';
   	    //Div contenant les champs hidden permettant de passer les valeurs sélectionner
   	    $toReturn .= '<div id="hidden_'.$id.'" style="display:none;position:absolute">'.$hidden.'</div>';
	    return $toReturn;
    }
}
?>