			...
			divinput.setStyle('border-top','1px solid transparent');
	 			...
	 			divinput.addEvent('click', function () {
	 				if (div.getStyle('visibility') != 'visible') {
	 					...
	 					if (div.getSize().x < divinput.getSize().x) {
	 						div.setStyle('width',divinput.getSize().x);
	 					}
	 					div.fixdivShow();
	     				input.testZone ( divinput.getTop()-5, divinput.getLeft()-5, divinput.getSize().y+div.getSize().y+10,div.getSize().x+10 );
	 				} else {
	 					div.setStyles({
	     			    	'visibility':'hidden'
	     				});
	     		...
	     		input.addEvent('reset' , function (e) {
						...
						var value = '';
		  					...
		  					$$('.multipleselect_check_'+id).each ( function (elem) {
		  						if (elem.checked) {
		  							...
		  							$('hidden_'+id).set ('html', $('hidden_'+id).innerHTML+'<input type=\"hidden\" name=\"'+$('name_'+id).getProperty('rel')+'[]\" value=\"'+elem.value+'\" />');
		  						}
		  					});
		  					input.value = value;
					}
