					...
					if (comment != '') {
						...
						for (z=0; z<bits.length; z++) {
							...
							if (pcs.length == 2) {
								...
								var newPcs = pcs.slice(1, pcs.length)
								newPcs = newPcs.join(':');
								con[pcs[0].trim()] = newPcs.trim();
							} 
						}
					}
	...
	*/
	...
	getImg: function(name) {
		...
		if (name != '') {
			...
			html += ' style="background-image:url(' + img + '); background-position:-' + (i*18) + 'px 0px;background-repeat:no-repeat;"';
		}
